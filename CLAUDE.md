...
- **Signaling API contract (Spreed 20+, найдено на spike-gate 2026-07-20):** signaling = **v3** (`/api/v3/signaling/{token}` pull GET / send POST; `/api/v3/signaling/settings` **БЕЗ token**), call = **v4** (`/api/v4/call/{token}`), joinRoom = `POST /api/v4/room/{token}/participants/active`. Flow: joinRoom → pull v3 → joinCall v4 (**pull ПЕРЕД joinCall** — session создаётся pull'ом). `pullMessages` требует PHP `$_SESSION` (Basic-auth её не даёт → нужен web-login flow для API-клиента). Наш код шлёт v4 signaling и JoinCall первым — **БАГ, скрытый compile-only**. Источник: `custom_apps/spreed/appinfo/routes/`. Проверять на Docker-сервере, НЕ compile-only.

**Статус:** spike реализован (Этапы 0-2), code-review пройден (9 valid правок, `8a8684b`), тесты зелёные. **Spike-gate attempt (2026-07-20) ПРОВАЛЕН из-за багов signaling-слоя** (НЕ из-за HPB — ранняя версия вывода была ошибочной: проверялся v4, а Spreed использует v3). **Compile-only скрыл 5 фундаментальных несоответствий** реальному Spreed internal-signaling flow. Подход валиден (signaling pull v3 реально работает — подтверждено access.log'ом браузера на Docker-сервере Talk 20.1.11), но signaling layer требует переработки. Полный отчёт + canonical flow: `docs/session-state/2026-07-19-nctalk-audio-calls.md`. Переработка — через `/spec-to-code` с **обязательной** проверкой на Docker-сервере (не compile-only).
