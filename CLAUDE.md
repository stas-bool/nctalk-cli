...
**Makefile**: `make build` / `make vet` / `make test` — обёртки под `CGO_ENABLED=0`; для базового `nctalk` достаточно и команд выше.
...
- **`internal/transport` / `internal/room` / `internal/exit`** — общие примитивы: HTTP/OCS-транспорт (`DoOCS`, конверт `ocs.data`, Basic-auth + OCS-заголовки, redirect-политика), `ResolveRoom` и exit-контракт `0/1/2/3` (value-тип `ExitError`: `errors.As` с value-target). Появились при рефакторинге под звонки — звонки удалены (2026-09-24), примитивы остались: `client` делегирует через alias.
...
`docs/superpowers/` — `specs/` (эталон контракта: `2026-07-17-nctalk-cli-design.md` — базовый CLI; `2026-09-08-nctalk-chat-edit-design.md` — `chat edit`; `2026-09-09-nctalk-rooms-participants-design.md` — `rooms participants`; исторические — спеки удалённого эксперимента аудио-звонков: `2026-07-19-nctalk-call-design.md`, `2026-07-21-nctalk-talk-tui-design.md`, `2026-09-23-nctalk-hpb-signaling-design.md`), `plans/` (декомпозиция реализации), `reviews/` (code-review). `docs/integration-run.md` — запуск интеграционных тестов.
